...
- **P1 Search offload** — never hand-grep. `ripgrep` → `ast-grep`/Probe → semantic (`mgrep`) only for concept queries, last resort.
...
- **P3 Verification gate** — no state change from unverified navigation. Before a multi-file change, re-confirm the target in one shot + compile/test.
...
- **P5 Persistent memory** — record only verified findings, after human review, to files. No auto-execution, no blind auto-injection.
...
- **P8 Output compression** — no verbose prose, no whole-file re-prints. Compressed output; search results and diffs in minimal form.

## Tooling posture
Deterministic CLI first: `ripgrep` (lexical) → `ast-grep`/Probe (structural) → Serena (symbol/LSP) → semantic last. **These are optional — their absence must not break base operation.** Keep MCP minimal — **none loaded by default**; Serena (LSP) is the only opt-in MCP. Reasoning uses native extended thinking; research uses native WebSearch; current library docs via Context7/`ctx7` when present — don't trust stale API memory.
