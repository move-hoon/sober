...
*   **L3 심볼:** Serena (LSP)
*   **L3.5 구조:** GitNexus CLI (조건부 구조 힌트, 깊게 읽기 전 `rg`/Probe 검증 필수, MCP 기본 비활성)
...
| `structure-graph` | 큰/낯선 레포에서 흐름·의존성·영향 범위가 불명확할 때 | "GitNexus CLI로 후보만 좁히고, 깊게 읽거나 수정하기 전 반드시 `rg`/Probe로 검증해. MCP는 기본으로 붙이지 말고, Spring DI/AOP/Event 같은 런타임 배선은 테스트나 프레임워크 근거로 확인해." |
...
| 도구 | 언제 사용하는가 | 역할 | 경계 (하지 말아야 할 것) |
|---|---|---|---|
| `ripgrep` | 키워드, 정확한 토큰명 또는 정규식 패턴 검색 시 | 빠른 정확 텍스트 검색 | 시맨틱/개념 검색에 사용 금지 |
| `ast-grep` | 기계적이거나 다수의 파일에 걸친 반복적인 코드 구조 수정 시 | 코드 형태 기반 기계적 재작성 | 미리보기 가능한 rewrite에 사용; 구조적 레포 검색은 Probe 사용 |
| Probe | 호출부/정의 탐색 및 구조적 코드 패턴 파악 시 | 인덱스 없는 구조적 레포 검색 | 읽기 전용; 코드 수정 불가 |
| Serena | 타입 기반의 단일 심볼 수정(이름 변경, 메서드 본문 변경 등) 시 | LSP 기반 심볼 탐색과 편집 | 런타임 DI/AOP 배선 파악 불가 |
| Context7 / `ctx7` | 외부 라이브러리 API 또는 종속성 문서 조회 시 | 오래된 API 기억 대신 최신 라이브러리 문서 | 프로젝트 내부 비즈니스 로직에 사용 금지 |
| `gitnexus` | 대형/낯선 레포에서 진입점, 호출 흐름, 의존성, 영향 범위 후보를 좁힐 때 | CLI 기반 정적 구조 그래프 후보 생성기 | MCP 상시 연결은 기본 비활성. `gitnexus analyze --skip-agents-md --skip-skills --skip-embeddings`로 Sober spine/skills와 비용을 보호하고, 후보는 깊게 읽거나 수정하기 전 `rg`/Probe로 검증 |
| `mgrep` | 토큰명은 모르지만 고수준의 개념적 탐색이 필요할 시 | 개념 질의용 시맨틱 검색 (최후 수단) | 키워드/정확도 매칭에 사용 금지 (CoREB 한계) |
