...
기존 설정은 보존합니다. Sober는 자신이 관리하는 블록, 심볼릭 링크, 훅, 규칙만 추가하거나 갱신하며, 제거 시에는 Sober가 만든 파일과 링크만 삭제합니다.
...
### 사전 요구사항
- **운영체제(OS)**: macOS 또는 Linux. Windows 사용자는 WSL(Windows Subsystem for Linux)을 사용해야 합니다.
- **에이전트**: [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 또는 [Codex CLI](https://github.com/openai/codex) 설치 완료
- **런타임**: Node.js 18 이상
...
Sober를 통해 AI 에이전트의 작업 규율을 확립하는 가장 빠른 방법입니다.

#### **Step 1: 글로벌 설치 및 도구 구성 (Global Installation & Setup)**
두 가지 방식 중 개발 환경에 적합한 설치 모드를 선택하여 진행합니다:

##### **옵션 A: 대화형 전체 구성 (초기 설치 시 강력 권장)**
Sober 핵심 정책/훅 배포뿐 아니라, `jq`, `ripgrep`, `ast-grep`, `Probe` 및 `Context7` 등 권장/보조 도구들의 설치 상태를 원클릭으로 점검하고 대화형 설치 가이드를 제공합니다.
```bash
npm install -g getsober@latest
sober setup
```

##### **옵션 B: 설정 파일만 업데이트 (조용한 설치)**
이미 관련 보조 도구들이 설치되어 있거나, 기존 전역 설정(`~/.sober`)에 정책과 스킬 및 훅 파일만 가볍게 동기화하고자 할 때 사용합니다.
```bash
npm install -g getsober@latest
sober install
```
...
#### **Step 2: 로컬 프로젝트 적용 (Local Project Integration)**
Sober를 적용하고자 하는 프로젝트 루트 디렉터리로 이동하여 행동 강령(`AGENTS.md`) 및 세션 핸드오프용 파일(`HANDOFF.md`)을 생성합니다.
```bash
cd your-project
...
#### **Step 3: 진단 및 유지보수 (Diagnostics & Maintenance)**
Sober 툴킷의 상태를 진단하거나, 완전히 제거해야 할 때 다음 명령어를 사용합니다:

- **환경 진단 및 상태 점검**: 설치된 훅, 스킬, 의존성 도구들의 상태를 상세히 출력합니다.
  ```bash
  sober doctor
  ```
- **Sober 완전 제거**: 기존의 사용자 설정을 훼손하지 않고, Sober 소유의 심볼릭 링크, 관리되는 훅 항목, 그리고 전역 디렉터리(`~/.sober`)를 안전하게 완전 제거합니다.
  ```bash
  sober uninstall
  ```
...
Sober는 전역과 로컬 두 개의 명확한 범위(Scope)로 나뉘어 설치 및 관리됩니다:

* 🏠 **글로벌 홈 범위 (`~/`)** - `sober setup` 또는 `sober install`로 생성
  * `~/.sober`: 정책, 훅, 스킬 등을 보관하는 Sober의 단일 진실 공급원(Source of Truth) 디렉터리.
  * `~/.claude` & `~/.codex`: Sober의 행동 지침 및 설정이 안전하게 머지(Merge)되는 설정 경로.
* 📁 **로컬 프로젝트 범위 (`./`)** - `sober template`으로 생성
  * `[project]/AGENTS.md`: 프로젝트 전용 에이전트 행동 강령.
  * `[project]/HANDOFF.md`: 세션 간의 작업 연속성을 유지하기 위한 로컬 요약 메모리.
...
Sober의 중심은 하나의 공유 `AGENTS.md`입니다. 이는 Claude Code와 Codex CLI가 에이전트 지침을 찾는 경로에 Sober가 설치하는 작업 계약입니다. 에이전트의 기본 행동을 더 엄격한 작업 루프로 통제합니다:
