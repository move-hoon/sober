...
Sober는 모델, MCP 도구, 커스텀 명령어, 서브에이전트와는 다른 계층(layer)에 위치합니다.

| 계층                    | 역할                            | Sober와의 관계                                                    |
| ----------------------- | ------------------------------- | ----------------------------------------------------------------- |
| Claude Code / Codex CLI | AI 코딩 에이전트 실행           | Sober는 에이전트를 대체하지 않고, 검증 가능한 작업 흐름 안에서 일하게 합니다. |
| MCP 도구               | 에이전트가 접근할 수 있는 영역 확장| Sober는 도구 사용의 순서와 기준을 잡아줍니다.                    |
| 커스텀 명령어           | 특정 작업 전용 워크플로우 추가  | Sober는 해당 워크플로우들이 시작되는 기본 행동 계층 역할을 합니다. |
| 서브에이전트             | 작업 분할 또는 위임             | Sober는 각 에이전트가 작업에 접근하는 방식을 결정합니다.          |
| 프롬프트 팩             | 채팅에 복사/붙여넣기 하는 지침  | Sober는 로컬 설정 및 프로젝트 파일에 설치되는 구조적 규칙입니다.  |

비유하자면, Sober는 AI 코딩 에이전트를 위한 **제어 하네스이자 로컬 작업 계약**입니다. 새 에이전트를 만드는 것이 아니라, 기존 Claude Code와 Codex CLI가 먼저 찾고, 작게 수정하고, 검증하고, 핸드오프를 남기는 차분한 루프 안에서 움직이도록 붙잡아줍니다.
...
이 루프는 두 런타임 모두 읽을 수 있는 단일 규칙 파일인 [`AGENTS.md`](AGENTS.md)에 정의되어 있습니다. Claude Code는 `CLAUDE.md`를 통해, Codex는 `AGENTS.md`를 직접 읽습니다.
...
- **L0 출력:** Caveman 출력 압축 + 최신 외부 라이브러리 문서를 위한 Context7.
- **L1 검색:** ripgrep (`| head`) → 구조 검색을 위한 Probe → 개념/시맨틱 검색용 mgrep (최후 수단).
- **L2 편집:** 기계적 편집용 ast-grep `--rewrite` + 타입 인지 편집을 위한 Serena `replace_symbol`.
- **L3 심볼:** LSP 기반 심볼 탐색용 Serena.
- **L3.5 구조:** 조건부 구조 힌트용 GitNexus CLI; 깊게 읽기 전에 rg 또는 Probe로 후보 검증 필수. MCP 상시 연결은 기본 비활성.
- **L4 검증 & 격리:** 1샷 검증 → 컴파일/테스트 → 예산 한도 준수 → 네이티브 worktree 격리.
- **L5 메모리:** 정적 규칙을 위한 `AGENTS.md` / `CLAUDE.md`, 아키텍처 사실 기록을 위한 `.serena/memories`, 세션 상태 유지를 위한 `HANDOFF.md`.
- **L6 관측:** `/context`, `/cost`, `/status` 및 KPI 로그 점검.
