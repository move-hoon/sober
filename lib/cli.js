...
// Deterministic/conditional substrate. Absence is WARN, never FAIL — base operation
...
const OPTIONAL_TOOLS = [
  {
    ...
    commands: ["rg"],
    policy: "P1",
    role: "Fast exact text search",
    install: "brew install ripgrep   # or: apt-get install -y ripgrep",
  },
  {
    key: "ast-grep",
    commands: ["ast-grep"],
  ...
  },
  {
    ...
    policy: "P2",
    role: "Symbol/LSP navigation and type-aware edits (single allowed MCP)",
  ...
  },
  {
    ...
    commands: ["gitnexus"],
    policy: "P1",
    role: "Conditional CLI structure graph; hints only for large/unfamiliar repos, verify with rg/Probe before deep reading/editing",
...
function soberLogDir(home = process.env.HOME || "") {
  return path.join(home, ".sober", "logs");
}

function hasOption(argv, longName, shortName) {
  return argv.some((arg) => arg === longName || arg === shortName || arg.startsWith(`${longName}=`));
}
...
function parseAgentArgs(argv) {
  ...
  return { passthrough, yolo };
}

function buildCodexCommand(argv, home) {
  const { passthrough, yolo } = parseAgentArgs(argv);
  const args = [];

  if (yolo) {
    args.push("--yolo");
  } else if (!hasOption(passthrough, "--sandbox", "-s")) {
    args.push("--sandbox", "workspace-write", "--add-dir", soberLogDir(home));
  }
...
function buildClaudeCommand(argv) {
  const { passthrough, yolo } = parseAgentArgs(argv);
  const args = [];
...
function buildAgentCommand(agent, argv, home = process.env.HOME || "") {
  switch (agent) {
    case "codex":
      return buildCodexCommand(argv, home);
    case "claude":
      return buildClaudeCommand(argv);
    default:
      throw new Error(`unknown agent: ${agent}`);
  }
}

function runAgent(agent, argv) {
  if (!commandExists(agent)) {
    console.error(`${agent} not found. Run 'sober doctor' for install hints.`);
    return 127;
  }
  const { command, args } = buildAgentCommand(agent, argv);
...
function printOptionalToolsStatus(tools) {
  ...
  for (const tool of tools) {
    ...
    if (level === "PASS") {
      console.log(`  ${colorize("✓", "32")} ${tool.key.padEnd(18)} PASS  [${tool.policy}] ${tool.role}`);
    } else {
      warnings++;
      console.log(`  ${colorize("⚠", "33")} ${tool.key.padEnd(18)} WARN  [${tool.policy}] ${tool.role} — not installed`);
...
function inspectContext7Status(homeDir) {
  const claudeSkillPath = path.join(homeDir, ".claude", "skills", "find-docs", "SKILL.md");
  const universalSkillPaths = [
    path.join(homeDir, ".agents", "skills", "docs", "SKILL.md"),
    path.join(homeDir, ".agents", "skills", "find-docs", "SKILL.md"),
  ];
  ...
  return {
    binaryInstalled,
    version,
    claudeSkillInstalled: fs.existsSync(claudeSkillPath),
    universalSkillInstalled: universalSkillPaths.some((p) => fs.existsSync(p)),
...
function printContext7DoctorStatus(status) {
  ...
  console.log(
    `  ${status.claudeSkillInstalled ? colorize("✓", "32") : colorize("○", "33")} Claude skill      ${
      status.claudeSkillInstalled ? "find-docs installed" : "not installed"
    }`,
  );
  console.log(
    `  ${status.universalSkillInstalled ? colorize("✓", "32") : colorize("○", "33")} Universal skill   ${
      status.universalSkillInstalled ? "~/.agents skill installed" : "not installed"
    }`,
  );
  console.log(
  ...
  const fullyInstalled =
    ...
    status.claudeSkillInstalled &&
    status.universalSkillInstalled &&
    status.ruleInstalled;
  ...
  if (fullyInstalled) {
    console.log("             Context7 CLI plus Claude and Universal skills are fully installed.");
    console.log("");
    return { failed: false };
  }
  ...
  if (partiallyInstalled) {
    ...
    console.log("                  ctx7 setup --cli --universal");
    console.log("");
...
function hookCommandToManagedRef(command, homeDir) {
  // Scan every token so wrapper forms like `bash ~/.sober/scripts/hooks/x.sh`
  // are validated too.
...
function collectHookRefs(hooks, homeDir, includeRef = () => true) {
  const refs = new Map();
  const walk = (node) => {
    ...
    for (const [key, value] of Object.entries(node)) {
      if (key === "command" && typeof value === "string") {
        const ref = hookCommandToManagedRef(value, homeDir);
        if (ref && includeRef(ref)) refs.set(ref.absolute, ref);
      } else {
        walk(value);
      }
    }
...
function inspectInstalledHooks(homeDir) {
  ...
  const allRefs = collectHookRefs(settings.hooks, homeDir);
...
function inspectCodexHooks(homeDir) {
  ...
  const soberScripts = `${path.sep}.sober${path.sep}scripts${path.sep}`;
  const allRefs = collectHookRefs(config.hooks, homeDir, (ref) => ref.absolute.includes(soberScripts));
  const missing = allRefs.filter((r) => !fs.existsSync(r.absolute)).map((r) => r.relative);
  const expected = [
...
function runTemplate(args) {
  ...
  const spinePath = path.resolve(__dirname, "..", ".sober", "AGENTS.md");
  ...
  if (!fs.existsSync(spinePath)) {
    console.error(`Sober policy spine not found: ${spinePath}`);
    return 1;
  }
  if (!fs.existsSync(templateDir)) {
    console.error(`Sober project templates not found: ${templateDir}`);
    return 1;
  }
...
function isSoberSymlink(p, soberHome) {
  ...
  try {
    ...
    const absoluteTarget = path.isAbsolute(target) ? target : path.resolve(path.dirname(p), target);
    return absoluteTarget === soberHome || absoluteTarget.startsWith(soberHome + path.sep);
  } catch {
    return false;
  }
}
...
Usage:
  ...
  sober codex [args...]  Run Codex with Sober's workspace-write/log-dir preset
  sober claude [args...] Run Claude Code through Sober's launcher
...
Presets:
  --yolo                 Bypass approvals/permissions; isolated environments only
`);
}
...
function runCli(argv) {
  ...
  switch (cmd) {
    ...
    case "doctor":
      return runDoctor();
    case "codex":
      return runAgent("codex", argv.slice(1));
    case "claude":
      return runAgent("claude", argv.slice(1));
    case "uninstall":
      return runUninstall();
    case "template":
